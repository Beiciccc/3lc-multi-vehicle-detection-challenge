#!/usr/bin/env python3
"""Train a YOLOv8n-from-scratch train+val baseline and export submissions.
...
def make_trainval_dataset(starter_dir: Path, work_dir: Path) -> tuple[Path, dict]:
    merged = work_dir / "merged_trainval"
    img_out = merged / "data/train/images"
    lbl_out = merged / "data/train/labels"
    img_out.mkdir(parents=True, exist_ok=True)
    lbl_out.mkdir(parents=True, exist_ok=True)
    counts = {"train": 0, "val": 0}
    for split in ("train", "val"):
        img_dir = starter_dir / "data" / split / "images"
        lbl_dir = starter_dir / "data" / split / "labels"
        for img in sorted(img_dir.iterdir()):
            if not img.is_file():
                continue
            stem = f"{split}__{img.stem}"
            out_img = img_out / f"{stem}{img.suffix.lower()}"
            out_lbl = lbl_out / f"{stem}.txt"
            link_or_copy(img, out_img)
            src_lbl = lbl_dir / f"{img.stem}.txt"
            if src_lbl.is_file():
                link_or_copy(src_lbl, out_lbl)
            else:
...
def write_dataset_yaml(starter_dir: Path, work_dir: Path, out: Path) -> dict:
    merged, counts = make_trainval_dataset(starter_dir, work_dir)
    cfg = {
        "path": str(merged),
        "train": "data/train/images",
        "val": str(starter_dir / "data/val/images"),
        "test": str(starter_dir / "data/test/images"),
        "nc": 4,
        "names": CLASS_NAMES,
    }
    out.write_text(yaml.safe_dump(cfg, sort_keys=False), encoding="utf-8")
    return {"merged_dir": str(merged), "counts": counts}
...
def main() -> int:
    ...
    dataset_yaml = work_dir / "dataset_r103_trainval_nomix_close3.yaml"
    dataset_info = write_dataset_yaml(starter_dir, work_dir, dataset_yaml)

    print("=" * 80)
    print("R103 train+val low-confidence sweep")
    print(f"starter_dir={starter_dir}")
    print(f"dataset_yaml={dataset_yaml}")
    print(f"dataset_info={json.dumps(dataset_info, sort_keys=True)}")
    ...
    candidates = [
        ...
        {"round": "r106", "conf": 0.000075, "iou": 0.466125, "max_det": 300},
        {"round": "r107", "conf": 0.000060, "iou": 0.466125, "max_det": 300},
    ]
    summaries: list[dict] = []
    for cfg in candidates:
        tag = f"{cfg['round']}_trainval_nomix_close3_conf{cfg['conf']:.8f}_iou{cfg['iou']:.6f}".replace(".", "")
        ...
        summary = {
            ...
            "note": f"{cfg['round']} 2026-06-06: train+val YOLOv8n scratch, nomix close_mosaic=3, low confidence sweep",
            "rule_note": "single YOLOv8n checkpoint, yolov8n.yaml random initialization, provided competition train+val data only, 640 px, no TTA, no ensemble, no pseudo-labeling, no distillation",
        }
